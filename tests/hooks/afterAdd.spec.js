...
describe('hooks', () => {
  ...
  describe('-> afterAdd:', () => {
    let fn;
    const extension = {
      afterAddTest: (db, hooks) => {
        const handler = (...args) => fn(...args);
        hooks[utils.getName(db.user)].afterAddTask.push(handler);
        hooks[utils.getName(db.user)].afterAddTasks.push(handler);
        hooks[utils.getName(db.project)].afterAddUser.push(handler);
        hooks[utils.getName(db.project)].afterAddUsers.push(handler);
      },
    };
    let project;
    let user1;
    ...
    describe('-> 1:M relationship:', () => {
      ...
      it('should have added child objects correctly', async () => {

...
describe('hooks', () => {
  ...
  describe('-> beforeRemove:', () => {
    let fn;
    const extension = {
      beforeRemoveTest: (db, hooks) => {
        const handler = (...args) => fn(...args);
        hooks[utils.getName(db.user)].beforeRemoveTask.push(handler);
        hooks[utils.getName(db.user)].beforeRemoveTasks.push(handler);
        hooks[utils.getName(db.project)].beforeRemoveUser.push(handler);
        hooks[utils.getName(db.project)].beforeRemoveUsers.push(handler);
      },
    };
    let project;
    let user1;

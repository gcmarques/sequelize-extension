...
describe('hooks', () => {
  ...
  describe('-> beforeSet:', () => {
    let fn;
    const extension = {
      beforeSetTest: (db, hooks) => {
        const handler = (...args) => fn(...args);
        hooks[utils.getName(db.task)].beforeSetUser.push(handler);
        hooks[utils.getName(db.user)].beforeSetTasks.push(handler);
        hooks[utils.getName(db.project)].beforeSetUsers.push(handler);
      },
    };
    let project;
    let user1;
